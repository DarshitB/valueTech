...
function MachineryReport() {
  ...
  // Function to generate disclaimer with valuer name and bank/branch/city (same as CE report)
  const getDisclaimer = useCallback(
    (valuerName = "VALUETECH SOLUTIONS", orderForDisclaimer = null) => {
      ...
      let text = disclaimerTemplate.replace(/VALUETECH SOLUTIONS/g, valuerName?.trim() || "VALUETECH SOLUTIONS");
      // Replace bank, branch, city block (same pattern as CE report: bank_name, branch_name, city)
      const bankName = orderForDisclaimer?.bank_name?.trim() || "UGRO CAPITAL LIMITED";
      const branchName = orderForDisclaimer?.branch_name?.trim() || "KAROL BAGH BRANCH";
      const cityName = (orderForDisclaimer?.city || orderForDisclaimer?.city_name)?.trim() || "NEW DELHI";
      const bankBranchCity = `${bankName}, ${branchName}, ${cityName}`;
      text = text.replace(/UGRO CAPITAL LIMITED, KAROL BAGH BRANCH, NEW DELHI/g, bankBranchCity);
      return text;
    },
    []
  );
          ...
          disclaimer:
            prev.disclaimer && prev.disclaimer.trim() !== ""
              ? order?.valuer_name
                ...
                : prev.disclaimer
              : getDisclaimer(order?.valuer_name || "VALUETECH SOLUTIONS", order),
        };
      });
    }
  }, [
