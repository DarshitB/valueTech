...
function MachineryReport() {
  ...
  const getDisclaimer = useCallback(
    (valuerName = "VALUETECH SOLUTIONS", orderForDisclaimer = null) => {
      ...
      let text = disclaimerTemplate.replace(/VALUETECH SOLUTIONS/g, valuerName?.trim());
      // Replace bank, branch, city block (same pattern as CE report: bank_name, branch_name, city)
      const bankName = orderForDisclaimer?.bank_name?.trim();
      const branchName = orderForDisclaimer?.branch_name?.trim();
      const cityName = (orderForDisclaimer?.city || orderForDisclaimer?.city_name)?.trim();
